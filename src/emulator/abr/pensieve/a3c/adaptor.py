...
file_handler = logging.FileHandler("/mydata/logs/adaptor.log", mode="w")
...
class ActorNetwork(object):
    ...
    def __init__(self, sess, state_dim, action_dim, bitrate_dim, hidden_dim):
        ...
        self.network_params = \
            tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope='adaptor_actor')
        ...
        for param in self.network_params:
            ...
            logger.info(f"self.input_network_params append param shape: {param.shape}")
    ...
    def create_actor_network(self):
        with tf.variable_scope('adaptor_actor'):
            logger.info(f"create_actor_network self.s_dim type: {type(self.s_dim)}, value: {self.s_dim}")
    ...
    def set_network_params(self, input_network_params):
        logger.info(f"set_network_params input_network_params type: {type(input_network_params)}")
        for param in input_network_params:
            logger.info(f"param shape: {param.shape}")

        try:
            logger.info("Before running session to set network parameters")
            ...
            #     i: d for i, d in zip(self.input_network_params, input_network_params)
            # })
            logger.info("Successfully set network parameters")
        except Exception as e:
            logger.error(f"Error setting network parameters: {e}")
...
class CriticNetwork(object):
    ...
    def __init__(self, sess, state_dim, learning_rate, bitrate_dim, hidden_dim):
        ...
        # Get all network parameters
        self.network_params = \
            tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope='adaptor_critic')
    ...
    def create_critic_network(self):
        with tf.variable_scope('adaptor_critic'):
            # Now we assume s_dim is a single integer, so the input shape is [None, s_dim]
            inputs = tflearn.input_data(shape=[None, self.s_dim])
