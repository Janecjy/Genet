    ...
    def train(self, train_envs, save_dir, iters=1e5, use_replay_buffer=False, original_actor_path=None):
        ...
        with tf.Session() as sess, \
            ...
            actor = a3c.ActorNetwork(sess,
                                     state_dim=EMBEDDING_SIZE+HIDDEN_SIZE,
                                     action_dim=A_DIM,
...
def agent(agent_id, net_params_queue, exp_queue, train_envs,
    ...
    video_server_port = 6626 + agent_id  # unique server port for each agent
    ...
    bpftrace_out_path = f"/mydata/logs/bpftrace_out_{agent_id}.txt"
    ...
    with tf.compat.v1.Session() as sess:
        ...
        actor = a3c.ActorNetwork(sess, state_dim=EMBEDDING_SIZE+HIDDEN_SIZE,
                                 action_dim=A_DIM, bitrate_dim=BITRATE_DIM)
        critic = a3c.CriticNetwork(sess, state_dim=EMBEDDING_SIZE+HIDDEN_SIZE,
                                  learning_rate=CRITIC_LR_RATE,
                                  bitrate_dim=BITRATE_DIM)
        ...
        s_batch = [np.zeros(EMBEDDING_SIZE+HIDDEN_SIZE)]
        a_batch = [action_vec]
        r_batch = []
        entropy_record = []
        tokens = np.array([])
            ...
            if browser_active and int(browser_active) == 1:
                ...
                if recv_state:
                    ...
                    if end_of_video and int(end_of_video) == 1:
                        ...
                        selection = 0
                        action_vec[selection] = 1
                        s_batch.append(np.zeros(EMBEDDING_SIZE+HIDDEN_SIZE))
